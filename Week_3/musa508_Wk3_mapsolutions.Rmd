...
author: "Matthew Harris, Michael Fichman and Anna Duan"
date: "August, 2024"
output:
  html_document:
    toc: true
...
```{r}
ggplot() +
  geom_sf(data = studyAreaTowns, fill = "gray95", color = "gray70") +
...
ggplot() +
  geom_sf(data = studyAreaTowns, fill = "gray95", color = "gray70") +
  geom_sf(data = filter(buffersAndTowns, Legend == "Inside"), aes(fill = "Inside"), color = "transparent") +
  geom_sf(data = filter(buffersAndTowns, Legend == "Outside"), aes(fill = "Outside"), color = "transparent") +
  geom_sf_label(data = studyAreaTowns %>% 
                  group_by(MUNI) %>% 
                  summarise(), 
                aes(label = str_remove(MUNI," TOWNSHIP")), 
                size = 2) +
  labs(fill = "Buffer Section") +
  theme_void() +
  theme(
